...
"""
════════════════════════════════════════════════════════════════════════
  M130 SITL Flight Analysis — Interactive HTML Report
  تحليل نتائج طيران SITL — تقرير HTML تفاعلي احترافي
════════════════════════════════════════════════════════════════════════

Generates a self-contained professional HTML report with:
  - Key flight metrics dashboard
  - Interactive Plotly charts (zoom, hover, pan)
  - Dark/Light mode toggle
  - Tabbed interface: Trajectory, Attitude, Aero, Forces, Control, Actuators
  - Scoring system (PASS/WARN/FAIL)
  - Auto-diagnostics & recommendations

Usage:
    python sitl_analysis.py                        # latest SITL CSV
    python sitl_analysis.py --file <csv_path>      # specific file
    python sitl_analysis.py --no-open              # don't open browser
"""
...
def load_sitl_csv(path: Path) -> pd.DataFrame:
    """Load SITL bridge CSV and compute derived columns."""
    df = pd.read_csv(path)
    ...
    # Flight path angle: frame-independent.
    # v_h = sqrt(|v|² - climb_rate²) — صحيح في NED أو ECEF (rotation invariant).
    ...
    # G-load from body accelerations
    ...
    # Alt AGL
    df["alt_agl_m"] = df["altitude"]

    # Flight phase detection
    df["flight_phase"] = "COAST"
    # Detect boost: mass decreasing rapidly
    dm = np.gradient(df["mass"].values)
    df.loc[dm < -0.01, "flight_phase"] = "BOOST"
    # Terminal: descending
    df.loc[(df["altitude"] < df["altitude"].max() * 0.3) &
...
def extract_metrics(df: pd.DataFrame, filepath: Path) -> dict:
    m = {}
    t = df["time_s"].values
    last = df.iloc[-1]
    ...
    m["peak_alt_m"] = df["altitude"].max()
    m["peak_alt_time_s"] = df.loc[df["altitude"].idxmax(), "time_s"]
    m["impact_speed_mps"] = last["speed_total"]
    m["max_speed_mps"] = df["speed_total"].max()
    m["max_mach"] = df["mach"].max()

    # AoA/β عند vel<50 m/s = vacuous (atan(wind/0) on rail). فلتر القضيب +
    # rail-departure transient حيث α/β arctan2 من components صغيرة جداً
    # تنتج peaks وهمية (e.g., 13.7° beta عند t=0.5s مع رياح + thrust misalign).
    _inflight = df["speed_total"].values > 50.0
    if _inflight.any():
...
# Scoring thresholds — matched to advanced_analysis.py with SITL-appropriate adjustments
# Weights sum to 100.  impact_angle uses adaptive threshold based on trajectory type.
SCORE_WEIGHTS = {"range": 30, "impact_angle": 15, "stability": 20, "aoa": 15, "sideslip": 10, "g_load": 10}
THRESH = {
    "range_error_good":  50,   # m — absolute error
    "range_error_warn":  200,  # m (relaxed: SITL has bridge latency + PX4 control loop delay)
    "max_alpha_warn":    15,   # deg
    "max_alpha_fail":    25,   # deg
    "max_beta_warn":     9,    # deg (4 m/s crosswind on slow rocket yields β≈8°; was 5°)
    "max_beta_fail":     15,   # deg (raised proportionally; was 10°)
...
def score_run(m: dict) -> dict:
    """Score using logic aligned with advanced_analysis.py, adapted for SITL bridge data."""
    scores = {}
    W = SCORE_WEIGHTS

    # Range accuracy — يَستَخدم خطأ الضربة الكلي (downrange + cross-range)
    w = W["range"]
    ...
    if abs_err < THRESH["range_error_good"]:
        ...
        v = "FAIL"
    _dr = m["range_error_m"]
    _cr = m.get("cross_range_error_m", 0.0)
    scores["range"] = {"score": round(s, 1), "verdict": v,
                        "detail": f"miss = {miss_total:.0f}m  (DR {_dr:+.0f}, CR {_cr:+.0f})"}

    # Impact angle — adaptive threshold based on trajectory type
    # Flat trajectories (peak alt < 500m AGL): accept shallow γ down to -5°
    # High trajectories (peak alt > 500m AGL): expect steep dive -20° to -80°
    w = W["impact_angle"]
    gamma = m["impact_gamma_deg"]
    is_flat = m["peak_alt_m"] < 500  # flat trajectory
    if is_flat:
        # Flat trajectory: any negative γ is acceptable
        if gamma < -3:
            s, v = w, "PASS"
        elif gamma < 0:
    ...
    # Build flat checks list for HTML rendering
    checks = [(k, v["verdict"], v["detail"]) for k, v in scores.items() if not k.startswith("_")]
    return {"checks": checks, "details": {k: v["score"] for k, v in scores.items() if not k.startswith("_")},
...
def diagnose(df, m):
    ...
    if m["flight_time_s"] < 5:
        diags.append(("error", "Very Short Flight", f"Only {m['flight_time_s']:.1f}s — possible early crash or sim failure"))
    if m["max_alpha_deg"] > 15:
        diags.append(("warning", "High Angle of Attack", f"|α| reached {m['max_alpha_deg']:.1f}° — risk of stall/instability"))
    if m["max_omega_deg_s"] > 200:
        diags.append(("warning", "High Angular Rate", f"ω reached {m['max_omega_deg_s']:.0f}°/s — tumbling risk"))
    if m["max_fin_cmd_deg"] < 0.1:
        diags.append(("warning", "No Control Commands", "PX4 rocket_mpc sent near-zero fin commands — check ARM state"))
    if m["propellant_kg"] < 0.1:
        diags.append(("info", "No Propellant Burned", "Mass barely changed — check thrust model"))
    # Check fin saturation
    sat_pct = 0
    for i in range(1, 5):
        sat_pct = max(sat_pct, (df[f"fin_cmd_{i}_deg"].abs() > 19.5).mean() * 100)
    if sat_pct > 10:
        diags.append(("warning", "Fin Saturation", f"Fins at max deflection {sat_pct:.0f}% of the time"))

    # ── Servo delay compensation diagnostics (SITL) ────────────────────
    ...
    if delay_issues >= 2:
        diags.append(("error", "Servo Delay Under-Compensated",
                      "MPC commands arrive too late for servo — increase delay_steps "
                      "in rocket_properties.yaml OR set lookahead_stage in 6dof_config_advanced.yaml. "
                      "Symptoms: " + "; ".join(delay_hints)))
    elif delay_issues == 1:
        diags.append(("warning", "Possible Servo Delay Mismatch",
                      "One symptom of under-compensated delay: " + delay_hints[0]))
...
def recommend(m, scores, diags):
    ...
    if m["max_alpha_deg"] > 12:
        recs.append("High AoA detected — consider tighter pitch control gains or earlier guidance engagement")
    if m["pitch_std_last30pct"] > 5:
        recs.append("Pitch oscillation in terminal phase — check damping gains and actuator bandwidth")

    # ── Servo delay compensation recommendations (SITL) ────────────────
    max_roll = m.get("max_roll_deg", 0)
    pitch_std = m.get("pitch_std_last30pct", 0)
    max_alpha = m.get("max_alpha_deg", 0)
    ...
    if delay_issues >= 2:
        recs.append(
            "⚠ SERVO DELAY: Increase delay_steps in rocket_properties.yaml "
            "and ensure lookahead_stage is auto-computed (remove it from config). "
            "Rule: lookahead_stage = delay_steps × dt_sim / dt_h. "
            "For HIL/flight: set RKT_MPC_SVO_DLY = delay in seconds "
            "(e.g. 0.080 for 80ms)"
        )
    elif delay_issues == 0 and scores.get("overall") == "PASS":
        recs.append(
            "✅ Servo delay compensation OK — lookahead_stage matches delay_steps. "
            "For HIL/flight: set RKT_MPC_SVO_DLY to the measured servo delay in seconds."
        )

    if not diags and scores["overall"] == "PASS":
        recs.append("Flight looks nominal — no issues detected")
    return recs
...
PHASE_COLORS = {"BOOST": "#e74c3c", "COAST": "#3498db", "TERMINAL": "#e67e22"}
...
def _fig_aero(df):
    t = df["time_s"]
    fig = make_subplots(rows=2, cols=2,
                        subplot_titles=("Mach Number", "Dynamic Pressure & G-Load",
    ...
    fig.add_trace(go.Scatter(x=df["mach"], y=df["alpha_deg"], mode="markers",
                             name="α vs Mach", marker=dict(size=3, color=t, colorscale="Viridis",
...
def _fig_forces(df):
    t = df["time_s"]
    fig = make_subplots(rows=2, cols=2,
                        subplot_titles=("Body Forces (3-axis)", "Body Accelerations",
...
def _fig_control(df):
    ...
    # Command envelope: all 4 fins overlaid as scatter
...
def _fig_geo(df):
    """Ground track on map if lat/lon available."""
...
def _fig_phase_portrait(df):
    ...
    fig = make_subplots(rows=1, cols=2,
                        subplot_titles=("Pitch Phase Portrait (θ vs q)", "α Phase Portrait (α vs q)"))

    fig.add_trace(go.Scatter(
        x=df["pitch_deg"], y=df["omega_y_deg"], mode="markers",
        marker=dict(size=3, color=t, colorscale="Viridis", colorbar=dict(title="t (s)", x=0.45)),
...
(function(){
  var saved=localStorage.getItem('m130_sitl_theme')||'light';
  ...
  window.addEventListener('DOMContentLoaded',function(){
    ...
    btn.addEventListener('click',function(){
      ...
      localStorage.setItem('m130_sitl_theme',next);
...
def _metrics_table_html(m, scores=None, title="All Numerical Metrics"):
    ...
    order = ["Run Info","Impact","Trajectory","Aerodynamics","Loads",
             ...
             "Feedback","Commands","Actuators","Mass","Timing","Other"]
...
def _plotly_div(fig, height=None):
    if fig is None:
        return ""
...
def generate_html(df, metrics, scores, diags, recs, html_path=None):
    m = metrics
    ...
    <div class="grid grid-4" style="margin-bottom:16px">
      ...
      <div class="card">{_metric_card("Peak Alt", f"{m['peak_alt_m']:.0f}m",
    ...
    <div class="grid grid-4" style="margin-bottom:16px">
      <div class="card">{_metric_card("Max |α|", f"{m['max_alpha_deg']:.1f}°",
                                       "Angle of Attack")}</div>
      ...
      <div class="card">{_metric_card("Fin Max", f"{m['max_fin_cmd_deg']:.1f}°",
                                       f"Actual: {m['max_fin_act_deg']:.1f}°")}</div>
    ...
    if not diag_html:
        diag_html = '<div class="diag info"><div class="dtitle">All Clear</div><div class="ddetail">No issues detected</div></div>'
    ...
    <div id="tab-overview" class="tab-panel active">
      ...
      <div class="grid grid-3" style="margin-bottom:16px">
        {score_html}
        {checks_html}
        <div class="card"><h3 style="margin-bottom:8px">Diagnostics</h3>{diag_html}</div>
      </div>
    ...
    tab_ctrl = f'<div id="tab-ctrl" class="tab-panel"><h2>Control & Actuators (PX4 SITL)</h2>{_plotly_div(fig_ctrl)}</div>'
    ...
    toolbar = (
        ...
        '<span style="font-size:.8rem;color:var(--text-secondary)">SITL Bridge Flight Analysis</span>'
        '</div>'
    )
...
<title>M130 SITL Analysis &mdash; {html_escape(m['timestamp'])}</title>
{plotly_cdn}
<style>{_CSS}</style>
</head>
<body>
<div class="container">
  <h1>M130 SITL Flight Analysis</h1>
  {toolbar}
...
def print_console_summary(m, scores):
    ...
    print()
    print("═" * 58)
    print(f"  M130 SITL Analysis  {tag} {scores['overall']} ({scores['total']}/100)")
    print("═" * 58)
    print(f"  Range:     {m['impact_range_m']:.0f}m  (err {m['range_error_pct']:+.1f}%)")
    print(f"  Peak Alt:  {m['peak_alt_m']:.0f}m   Max Mach: {m['max_mach']:.3f}   Max G: {m['max_g']:.1f}")
    print(f"  Time:      {m['flight_time_s']:.2f}s   Max |α|: {m['max_alpha_deg']:.1f}°   Max fin: {m['max_fin_cmd_deg']:.1f}°")
    print(f"  Mass:      {m['mass_initial']:.2f} → {m['mass_final']:.2f} kg")
    # Servo delay compensation info
    max_roll = m.get("max_roll_deg", 0)
    pitch_std = m.get("pitch_std_last30pct", 0)
    max_alpha = m.get("max_alpha_deg", 0)
    ...
    if delay_issues >= 2:
        ...
        print(f"    → For HIL/flight: set RKT_MPC_SVO_DLY = measured delay (e.g. 0.080)")
    else:
...
def analyze_sitl_csv(csv_path, open_browser=True):
    csv_path = Path(csv_path)
    df = load_sitl_csv(csv_path)
    metrics = extract_metrics(df, csv_path)
    ...
    html_path = out_dir / f"sitl_analysis_{metrics['timestamp']}.html"

    generate_html(df, metrics, scores, diags, recs, html_path=str(html_path))
...
def discover_sitl_csvs(results_dir: Path) -> list:
    return sorted(results_dir.glob("sitl_*.csv"), key=lambda p: p.stat().st_mtime, reverse=True)


def main():
    parser = argparse.ArgumentParser(description="M130 SITL Flight — Interactive HTML Analysis")
    ...
    if args.file:
        ...
        csv_files = discover_sitl_csvs(_RESULTS_DIR)

    if not csv_files:
        print("ERROR: no sitl_*.csv files found in", _RESULTS_DIR)
        sys.exit(1)

    print(f"  Analyzing: {csv_files[0].name}")
    analyze_sitl_csv(csv_files[0], open_browser=not args.no_open)
