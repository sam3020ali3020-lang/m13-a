...
HIL_GPS_LEN = 36             # lat,lon,alt + velocities + fix info
...
# ============================================================================
# Lab Bridge — PX4 SITL + REAL servos via CAN (closed-loop)
# ============================================================================
#
# Fork of /sitl/mavlink_bridge.py with two modifications for closed-loop /lab:
#   1. Forces `simulation.use_actuator_dynamics = false` so the simulator does
#      NOT apply its own τ_servo (the real CAN servo already provides physical
#      τ — doubling causes alpha blow-up like in /hil).
#   2. `bridge_control()` returns the LATEST measured fin angles from CAN
#      feedback (via `_fin_provider`) instead of the raw MPC command, so the
#      simulator physics use the actual servo positions.
#
# Set `bridge._fin_provider = can_adapter.get_latest_fb_rad` after construction.
# Same actuator_callback hook as /sitl is preserved for forwarding cmds to CAN.

class LabBridge:
    """MAVLink bridge for /lab: PX4 SITL ↔ 6DOF sim ↔ REAL CAN servos."""
    ...
    def __init__(self, sitl_config_path: str, sim_config_path: Optional[str] = None):
        ...
        # /lab closed-loop: real CAN servos provide physical τ. Disable the
        # simulator's internal actuator model to avoid double-lag (same as /hil).
        sim_config['simulation']['use_actuator_dynamics'] = False
        ...
        # /lab closed-loop hook: callable returning the actual fin angles
        # (rad, length-4) measured from CAN. When set (and returns a valid
        # array), bridge_control() returns these REAL servo positions instead
        # of the raw MPC command, so simulator physics use real servo state.
        # Signature: provider() -> np.ndarray of shape (4,) in rad, or None.
        self._fin_provider = None
    ...
    def run(self, csv_output: Optional[str] = None) -> dict:
        ...
        # Control function: closed-loop /lab uses REAL CAN feedback if
        # available, otherwise falls back to the MPC command. The simulator
        # has `use_actuator_dynamics=false` so this fin angle goes straight
        # to aerodynamics without any additional simulated lag.
        def bridge_control(state_dict, t_sim):
            if self._fin_provider is not None:
                try:
                    fb = self._fin_provider()
                    if fb is not None:
                        arr = np.asarray(fb, dtype=float)
                        if arr.shape == (4,) and np.all(np.isfinite(arr)):
                            return arr
                except Exception:
                    pass  # fall back to MPC cmd if provider fails
        ...
        while step < n_steps and self._running:
            ...
            # ─── Hybrid Lockstep (مطابق لـ /hil) ────────────────────────────
            # في /lab السيرفوهات CAN فيزيائية لا يمكن إيقافها، لذلك
            # wall-clock pacing إجباري (مثل /hil). لكن حلقة sim↔PX4 الرقمية
            # تعاني TCP/scheduler jitter — lockstep wait يُزيله. النتيجة:
            # ratio = 1.0x (real-time للسيرفوهات) + دقة digital loop.
            if self.timing == 'lockstep':
                # In lockstep: wait for PX4 to respond (digital loop sync)
                ...
                # Real-time: non-blocking receive (wall-clock pacing below)
            ...
            # ── wall-clock pacing (كلا الوضعين) ────────────────────────────
            # السيرفوهات CAN فيزيائية تعمل بـ real-time — لا نسبق الزمن
            # الحقيقي. في lockstep: digital loop مُزامن + wall-clock pacing
            # = Hybrid Lockstep (مطابق لـ /hil). ratio متوقع ≈ 1.0x.
...
def main():
    ...
    bridge = LabBridge(args.sitl_config, args.sim_config)
