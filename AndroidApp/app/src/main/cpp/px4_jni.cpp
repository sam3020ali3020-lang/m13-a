...
static void stop_modules_only() {
    ...
    // Watchdog OFF first so it doesn't race the teardown trying to

    // auto-restart a module that's deliberately being stopped.  It will

    // be re-initialised at the end of start_px4_modules().
...
static void start_px4_modules(const std::string& storage_path) {
...

    {
        ...
        // SYS_AUTOSTART: default 22004 (HITL) on first run — أنسب للتطوير

        // والاختبار (PIL/HITL مع 6DOF sim). للطيران الحقيقي بدّل إلى 22005

        // عبر MAVLink PARAM_SET أو واجهة المستخدم.
        ...
        if (first_rocket_run || airframe_changed) {
            ...
            // ACC_NOISE must be HIGH to prevent GPS velocity innovations from

            // corrupting attitude through Kalman cross-terms during high-G burn.

            // With 0.5: K_vel≈2% → velocity error accumulates → dpitch=-4.5°

            // With 3.0: K_vel≈44% → GPS corrects velocity, not attitude.

            p = param_find("EKF2_ACC_NOISE");

            if (p != PARAM_INVALID) { float v = 2.0f; param_set(p, &v); }    // 3.0 → 2.0 (matches SITL airframe 22003 — verified 91/100)

            p = param_find("EKF2_GYR_NOISE");

            if (p != PARAM_INVALID) { float v = 0.025f; param_set(p, &v); }  // was 0.05

            p = param_find("EKF2_GPS_V_NOISE");

            if (p != PARAM_INVALID) { float v = 0.5f; param_set(p, &v); }   // 1.5 → 0.5 (matches SITL — bridge gps_vel_std=0.1)

            p = param_find("EKF2_GPS_P_NOISE");

            if (p != PARAM_INVALID) { float v = 2.5f; param_set(p, &v); }   // 2.0 → 2.5 (matches SITL — bridge gps_pos_std=2.5)
            ...
            if (p != PARAM_INVALID) { float v = 100.0f; param_set(p, &v); } // 200 → 100 (matches SITL + bridge gps_delay_ms=100)
            ...
            if (p != PARAM_INVALID) { float v = 0.25f; param_set(p, &v); }  // was 0.5 — fast tracking, tuned 2026-05-05
            ...
            // Midpoint between defensive defaults and Allan-derived walk rate.

            // Measured Allan BI (2026-05-03): accel 0.047 mg → walk ≈5e-5 m/s³,

            //                                 gyro  8.02°/h → walk ≈2e-6 rad/s².

            // Kept 100× margin for thermal drift + unmodeled correlation.

            p = param_find("EKF2_ACC_B_NOISE");

            if (p != PARAM_INVALID) { float v = 0.005f; param_set(p, &v); }  // was 0.02

            p = param_find("EKF2_GYR_B_NOISE");

            if (p != PARAM_INVALID) { float v = 0.0001f; param_set(p, &v); } // was 0.001 — freeze gyro bias, tuned 2026-05-05

            p = param_find("EKF2_ABL_LIM");

            if (p != PARAM_INVALID) { float v = 0.8f; param_set(p, &v); }   // 1.0 → 0.8 (PX4 max bound; avoids silent clipping)
            ...
            if (p != PARAM_INVALID) { float v = 50.0f; param_set(p, &v); }  // 10 → 50 (matches SITL — wider gate for high-G burn)
            ...
            // -- EKF2 معدل التنبؤ (100 Hz — يقلل CPU بالنصف؛ مطابق SITL airframe) --

            p = param_find("EKF2_PREDICT_US");

            if (p != PARAM_INVALID) { int32_t v = 10000; param_set(p, &v); } // 5000 → 10000 (200Hz → 100Hz)
            ...
            if (current_autostart == 22002 || current_autostart == 22005) {
                ...
                // ROCKET_SITL_GPS=0: real flight reads raw sensor_gps for guidance

                // origin and publishes to actuator_servos (not actuator_outputs_sim).

                // Force 0 even if a previous SITL session left it at 1.
                ...
                // ═══ RULE: RKT_MPC_SVO_DLY (servo transport delay) ════════════
                ...
                //   pure_delay = pure servo+CAN transport delay from /direct (ms)

                //   tau_servo  = servo first-order time constant from /direct (ms).

                //                The MPC's NX=15 ideal-fin model has NO servo

                //                dynamics — τ acts as ADDITIONAL effective dead

                //                time. WITHOUT τ in compensation, closed-loop

                //                exhibits a 5 Hz LIMIT CYCLE in pitch rate.

                //                (Verified 6DOF sim 2026-05-02.)

                //   + 40ms     = safety for delays /direct does NOT see:

                //                 • PX4 scheduling jitter (1–5 ms)

                //                 • sensor pipeline gyro→EKF2→MPC (5–15 ms)

                //                 • CAN bus contention w/ 4 actuators (1–3 ms)

                //                 • MPC solve-time variability (5–10 ms)

                //                 • future-planning safety (10–15 ms)

                //   100ms      = absolute floor: MPC needs lookahead_stage ≥ 5

                //               (= 5 × dt_h, dt_h=20ms) for stable dead-time

                //               compensation; below it MPC may oscillate.

                //

                //   How to set for a new servo:

                //     1. Run /direct repeatability → aggregate-fit

                //        Extract BOTH: pure_delay AND tau_servo (τ)

                //     2. candidate = pure_delay + tau_servo + 40 (ms)

                //     3. if candidate < 100 → use 0.100f (floor)

                //        else               → use candidate/1000.0f

                //     4. ALSO update the canonical definition at

                //        PX4-Autopilot/src/modules/rocket_mpc/rocket_mpc_params.c

                //        (PARAM_DEFINE_FLOAT(RKT_MPC_SVO_DLY, ...))

                //

                //   Current servo: XQPOWER XQ-BLS8145C

                //     /direct (2026-05-02, repeatability × 2 runs):

                //       pure_delay = 39.31 ms  (aggregate fit R²=0.95)

                //       tau_servo  = 29.88 ms  (aggregate fit R²=0.95)

                //     Apply rule: max(39.31 + 29.88 + 40, 100) = 109.19 ms

                //     → 0.109f → lookahead_stage = 5
                ...
                // EKF2_MAG_TYPE = 6 (init-only): align yaw from sim magnetometer
                // on the launch rail, then ignore mag in flight. Required for
                // ROCKET_USE_GT=0 (full EKF2 path); without it, yaw never aligns
                // and EKF2 NED frame is undefined → MPC sees garbage state.
                // Previous value (5 = none) only worked when ROCKET_USE_GT=1.
                p = param_find("EKF2_MAG_TYPE");
                ...
                // ROCKET_SITL_GPS: in HITL, _hitl flag already handles GPS source

                // routing and actuator path (via || in RocketMPC.cpp), so this

                // param is redundant here.  Set to 1 for documentation clarity.
                ...
                // SITL: use EKF2 lpos for guidance origin + route to actuator_outputs_sim.
...

        {
            ...
            struct { const char* name; int32_t id; bool skip_in_hitl; } cal_ids[] = {
                ...
                {"CAL_BARO0_ID", 6619141, false},  // ANDROID_DEVID_BARO ((0x65<<16)|5) — SERIAL bus → external (so PX4 uses real baro temp not 15°C default)
    ...
    // 7. Land detector — "fixedwing" mode for the rocket.

    //

    // Why not "multicopter"?

    //   MulticopterLandDetector relies on:

    //     (a) low thrust setpoint   ✓ (rocket post-burnout = thrust 0)

    //     (b) stable gyro/accel     ✗ rocket spins fast during descent

    //                                 → "stable" check never passes

    //                                 OR worse: triggers falsely at apogee

    //                                 if baro is noisy → mid-flight auto-disarm.

    //

    // Why "fixedwing" works:

    //   FixedwingLandDetector falls back to groundspeed/Z-velocity from EKF2

    //   when airspeed is invalid (which is our case — no pitot tube).  It uses

    //   rotational threshold ONLY when both airspeed and groundspeed are

    //   missing, so a spinning rocket during descent (with valid GPS velocity)

    //   stays "in air" until it actually stops moving.

    //

    // For SITL/HITL the detector is essentially advisory (sim ends before

    // landing) but for Real Flight (22005) this prevents the multicopter

    // false-trigger crash class.

    const char* ld_argv[] = {"land_detector", "start", "fixedwing", nullptr};
    ...
    if (sys_hitl == 1) {
        ...
        //      `commander arm -f` = force arm — يتجاوز preflight checks.

        //      هذا متعمّد في HITL لأن CPU load و missing-cal warnings تمنع

        //      pre_flight_checks_pass من الوصول لـ true على الأجهزة الأبطأ

        //      (OPPO mid-tier) رغم أن EKF2 متقارب وصالح للطيران المحاكى.
    ...
    // 14. Watchdog — must start LAST so every monitored module has had a

    //     chance to publish at least once before the first poll cycle.

    //     The log file lives next to PX4's other runtime artefacts so that

    //     /watchdog's runner can pull it via adb over the same channel as

    //     the PX4 log/ directory.

    {

        std::string wd_log = storage_path + "/watchdog_events.jsonl";

        if (wd_init(wd_log.c_str(), /*poll_period_ms=*/50)) {

            LOGI("Watchdog initialised (log=%s, period=50ms)", wd_log.c_str());

        } else {

            LOGE("Watchdog init failed — module liveness monitoring DISABLED");
...
// Watchdog JNI surface — see watchdog_native.h

//

// Layout of the long[] returned by nativeGetModuleStatus (one row per call):

//   [ 0] alive              (0/1)

//   [ 1] was_alive_ever     (0/1)
...
//   [12] age_us   (UINT64_MAX clamped to Long.MAX_VALUE for JVM — see code)

// =============================================================================



static jlong clamp_u64_to_jlong(uint64_t v) {

    // jlong is signed 64-bit; UINT64_MAX overflows.  Clamp to INT64_MAX so

    // Kotlin can still treat "never alive" as a very large age_us without

    // sign-flipping into a small negative.

    const uint64_t max_signed = 0x7FFFFFFFFFFFFFFFull;

    return (jlong)(v > max_signed ? max_signed : v);
...
Java_com_ardophone_px4v17_bridge_PX4Bridge_nativeGetModuleStatus(
    ...
    const bool ok = name ? wd_get_module_status(name, &st) : false;
    ...
    if (!ok) {
        ...
        // Return empty array to signal "unknown module / watchdog not running"
    ...
    jlong vals[13] = {
        ...
        st.was_alive_ever  ? 1 : 0,

        st.auto_restart    ? 1 : 0,

        (jlong)st.last_alive_us,

        (jlong)st.last_tick_us,

        (jlong)st.last_crash_us,

        (jlong)st.last_restart_us,

        (jlong)st.last_recover_us,

        (jlong)st.crash_count,

        (jlong)st.restart_count,

        (jlong)st.stale_us,

        (jlong)st.now_us,

        clamp_u64_to_jlong(st.age_us),

    };

    env->SetLongArrayRegion(out, 0, 13, vals);
...
Java_com_ardophone_px4v17_bridge_PX4Bridge_nativeListWatchdogModules(
    ...
    const size_t n = wd_list_modules(names, 32);



    jclass string_cls = env->FindClass("java/lang/String");

    jobjectArray out = env->NewObjectArray((jsize)n, string_cls, nullptr);

    for (size_t i = 0; i < n; ++i) {

        env->SetObjectArrayElement(out, (jsize)i, env->NewStringUTF(names[i]));
...
Java_com_ardophone_px4v17_bridge_PX4Bridge_nativeCrashModule(

    JNIEnv* env, jobject, jstring jname) {

    const char* name = env->GetStringUTFChars(jname, nullptr);

    const bool ok = wd_crash_module(name);
...
Java_com_ardophone_px4v17_bridge_PX4Bridge_nativeRestartModule(

    JNIEnv* env, jobject, jstring jname) {

    const char* name = env->GetStringUTFChars(jname, nullptr);

    const bool ok = wd_restart_module(name);
...
Java_com_ardophone_px4v17_bridge_PX4Bridge_nativeSetAutoRestart(

    JNIEnv* env, jobject, jstring jname, jboolean enable) {

    const char* name = env->GetStringUTFChars(jname, nullptr);

    const bool ok = wd_set_auto_restart(name, enable == JNI_TRUE);
...
Java_com_ardophone_px4v17_bridge_PX4Bridge_nativeGetWatchdogEvents(

    JNIEnv* env, jobject, jint maxEvents) {

    // 96 kB is enough for ~200 events of 500 bytes average.  Allocated on

    // the heap (not the stack) because JNI callers may be on small threads.

    constexpr size_t kBufSize = 96 * 1024;

    char* buf = (char*)malloc(kBufSize);

    if (!buf) return env->NewStringUTF("[]");

    wd_copy_recent_events(buf, kBufSize,

                          maxEvents > 0 ? (size_t)maxEvents : 0);
...
// Truncate the watchdog JSONL log in-process (close + reopen with O_TRUNC).

// Used by the /watchdog test runner between scenarios to reset the log

// without the "open-but-unlinked" race that `adb shell rm` introduces.

extern "C" JNIEXPORT jboolean JNICALL

Java_com_ardophone_px4v17_bridge_PX4Bridge_nativeTruncateWatchdogLog(

    JNIEnv*, jobject) {

    return wd_truncate_log() ? JNI_TRUE : JNI_FALSE;
