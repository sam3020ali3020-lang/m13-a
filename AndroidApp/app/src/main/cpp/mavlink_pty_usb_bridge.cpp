...
static std::thread g_th_pty_to_usb;
static std::thread g_th_usb_to_pty;
static int g_usb_fd{-1};
static int g_pty_master{-1};
static std::string g_slave_path;
static std::atomic<bool> g_threads_should_run{false};
...
static bool cp210x_configure(int fd, unsigned baud)
{
...
static void usb_release_if_needed(int fd)
{
	if (fd < 0) return;
	unsigned int iface = CP210X_USB_INTERFACE;
...
static int usb_bulk_write(int fd, const void *buf, int len)
{
...
static int usb_bulk_read(int fd, void *buf, int maxlen)
{
	struct usbdevfs_bulktransfer bulk {};
...
static void thread_pty_to_usb_fn()
{
	char buf[4096];
...
static void thread_usb_to_pty_fn()
{
	char buf[4096];
	while (g_threads_should_run.load()) {
		if (g_pty_master < 0 || g_usb_fd < 0) break;
...
static void bridge_stop_unlocked()
{
	...
	// Note: an earlier version called shutdown(g_pty_master, SHUT_RDWR) here
	// to wake the threads.  shutdown() is a sockets API and is undefined on a
	// PTY fd (returns -1 with errno=ENOTSOCK on Linux).  The wake is unneeded
	// anyway: thread_pty_to_usb_fn polls with a 100 ms timeout and the USB
	// read in thread_usb_to_pty_fn uses a 100 ms timeout too, so both
	// threads observe g_threads_should_run==false within ~100 ms.
