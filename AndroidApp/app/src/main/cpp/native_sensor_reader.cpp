...
static void* sensor_thread_func(void* /*arg*/) {
    ...
    uint64_t local_accel = 0, local_imu = 0, local_baro = 0, local_mag = 0;
    ...
    while (sensor_running) {
        ...
        while ((count = ASensorEventQueue_getEvents(queue, events, 64)) > 0) {
            for (int i = 0; i < count; i++) {
                ...
                switch (events[i].type) {
                    case ASENSOR_TYPE_ACCELEROMETER: case 35: local_accel++; break;
        ...
        if (elapsed >= 5) {
            SR_LOGI("RATES: Accel=%llu/s  Gyro=%llu/s  Baro=%llu/s  Mag=%llu/s",
                (unsigned long long)(local_accel / elapsed),
                (unsigned long long)(local_imu / elapsed),
                (unsigned long long)(local_baro / elapsed),
                (unsigned long long)(local_mag / elapsed));
            local_accel = local_imu = local_baro = local_mag = 0;
            rate_start = now;
        }
    }
