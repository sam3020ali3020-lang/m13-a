...
HIL_GPS_LEN = 36             # lat,lon,alt + velocities + fix info (base, no extensions)
HIL_STATE_QUATERNION_LEN = 64
HIL_ACTUATOR_CONTROLS_LEN = 81  # time(Q) + flags(Q) + 16 floats + mode(B)
...
def build_hil_gps(time_usec: int,
    ...
    payload = struct.pack('<QiiiHHHhhhHBB',
                          time_usec,
...
def _compute_body_accel(forces_body: np.ndarray, mass: float,
    ...
    forces_body = thrust + aero (from 6DOF sim, NO gravity).
    Newton: a = F_ext/m + g   →   specific_force = a - g = F_ext/m.
    """
    return forces_body / max(mass, 0.1)
...
class SITLBridge:
    ...
    def __init__(self, sitl_config_path: str, sim_config_path: Optional[str] = None):
        ...
        self.sensor_rate = bridge_cfg.get('sensor_rate_hz', 100)
        self.gps_rate = bridge_cfg.get('gps_rate_hz', 5)
        ...
        # Optional hook called whenever HIL_ACTUATOR_CONTROLS is received and
        # ``_last_controls`` is updated. Used by /lab to forward commands to a
        # real CAN bus alongside the simulated dynamics.
        # Signature: callback(controls: np.ndarray, t_sim_usec: int) -> None
        self._actuator_callback = None
        ...
        self._csv_output: Optional[str] = None
        self._prev_sigint = None
        self._prev_sigterm = None
        atexit.register(self.cleanup)

    def cleanup(self):
        """Deterministic cleanup: socket, temp file, CSV flush.
        ...
        # 2. Flush CSV if data exists and we haven't exported yet
        if self._csv_output and len(self.history.get('time', [])) > 0:
            try:
                self._export_csv(self._csv_output)
                logger.info("CSV saved on cleanup → %s", self._csv_output)
                print(f"[BRIDGE] CSV saved on cleanup → {self._csv_output}")
            except Exception as e:
                logger.warning("CSV export failed during cleanup: %s", e)

        # 3. Remove temp config
        try:
    ...
    def _connect(self):
        ...
        Raises ConnectionError if PX4 does not connect within 60s or if
        the port is unavailable (address already in use).
        """
        if self.protocol == 'tcp':
            ...
            try:
                ...
                server.close()
                msg = (f"Cannot bind TCP {self.host}:{self.port} — {e}. "
                       f"Port conflict? Kill stale process: "
            ...
            try:
                ...
                server.close()
                msg = (f"PX4 did not connect to TCP {self.host}:{self.port} "
                       f"within 60s — is PX4 running and configured for "
                       f"simulator_mavlink on this port?")
                logger.error(msg)
                print(f"[BRIDGE] ❌ {msg}")
            ...
            try:
                ...
                self._sock.close()
                self._sock = None
                msg = (f"Cannot bind UDP {self.host}:{self.port} — {e}. "
                       f"Port conflict? Kill stale process: "
                       f"fuser -k {self.port}/udp")
                logger.error(msg)
                print(f"[BRIDGE] ❌ {msg}")
                raise ConnectionError(msg) from e
    ...
    def run(self, csv_output: Optional[str] = None) -> dict:
        ...
        Returns:
            dict: Flight history
        """
        # Store csv path so cleanup() can flush data on interrupt
        ...
        def _signal_handler(signum, frame):
            ...
            print(f"\n[BRIDGE] {sig_name} received — shutting down gracefully...")
            logger.info("%s received — stopping bridge", sig_name)
            self._running = False

        self._prev_sigint = signal.signal(signal.SIGINT, _signal_handler)
        self._prev_sigterm = signal.signal(signal.SIGTERM, _signal_handler)
        ...
        # Note: with dt=0.01 the ceiling is 100 Hz (one HIL_SENSOR per sim step).
        ...
        # On a constrained pad: a=0, so F_ext = -m*g_body (constraint cancels gravity).
        # _compute_body_accel returns F_ext/m = -g_body → correct stationary IMU reading.
        ...
        pad_mass = state[13] if len(state) > 13 else 5.0
        pad_forces = -pad_mass * (C_ned2b @ np.array([0, 0, 9.80665]))
        initial_snapshot = {'forces': pad_forces, 'vel_ned': [0, 0, 0]}
        ...
        # Initial groundtruth state: rocket on pad at launch attitude
        # CRITICAL: Use actual launch quaternion so EKF2 correctly estimates
        # the launch pitch from accelerometer gravity alignment during warm-up.
        init_quat = state[6:10].copy()
        init_omega = np.zeros(3)
        # Compute gravity in body frame for the tilted launch attitude
        # This is what the accelerometer reads on the stationary launch pad
        g_ned = np.array([0.0, 0.0, 9.80665])
        q0, q1, q2, q3 = init_quat
        C_ned_to_body = np.array([
            [1 - 2*(q2*q2 + q3*q3), 2*(q1*q2 + q0*q3), 2*(q1*q3 - q0*q2)],
            [2*(q1*q2 - q0*q3), 1 - 2*(q1*q1 + q3*q3), 2*(q2*q3 + q0*q1)],
            [2*(q1*q3 + q0*q2), 2*(q2*q3 - q0*q1), 1 - 2*(q1*q1 + q2*q2)],
        ])
        # Specific force on pad = -gravity_body (accelerometer measures reaction to gravity)
        init_accel = -(C_ned_to_body @ g_ned)
        ...
        for init_i in range(boot_steps):
            ...
            # Recompute sensor noise each tick:
            # - Baro: prevents DataValidator STALE (100+ identical values)
            # - IMU: allows EKF2 to estimate gyro/accel bias from statistics
            #   (sending constant value causes EKF2 to lock bias = value+noise)
            noisy_alt = self.noise.add_baro_noise(self.launch_alt)
            noisy_baro = 1013.25 * (1.0 - 2.25577e-5 * noisy_alt) ** 5.25588
            # True pad readings: accel=-g_body, gyro=[0,0,0]
            noisy_accel, noisy_gyro = self.noise.add_imu_noise(
                sensors['accel_body_true'], np.zeros(3))

            # HIL_SENSOR LAST — triggers lockstep time advance in PX4
            self._send(build_hil_sensor(
                self._sim_time_usec,
                noisy_accel, noisy_gyro,
                sensors['mag_body'], noisy_baro,
                sensors['diff_pressure'], noisy_alt))
                    ...
                    for msg_id, payload in msgs:
                        if msg_id == MAVLINK_MSG_ID_HIL_ACTUATOR_CONTROLS:
                            ...
                            if parsed:
                                ...
                                if self._actuator_callback is not None:
                                    try:
                                        self._actuator_callback(self._last_controls, self._sim_time_usec)
                                    except Exception as _cb_err:
                                        logger.warning(f'actuator_callback raised: {_cb_err}')
                                if not got_controls:
                                    got_controls = True
        ...
        for arm_attempt in range(50):
            ...
            if arm_attempt % init_gps_every == 0:
                self._send(build_hil_gps(
                    ...
                    self.launch_lat, self.launch_lon, self.launch_alt,
                    0.0, 0.0, 0.0))
            # Recompute sensor noise each tick (same as boot loop)
            noisy_alt = self.noise.add_baro_noise(self.launch_alt)
            noisy_baro = 1013.25 * (1.0 - 2.25577e-5 * noisy_alt) ** 5.25588
            noisy_accel, noisy_gyro = self.noise.add_imu_noise(
                sensors['accel_body_true'], np.zeros(3))
        ...
        while step < n_steps and self._running:
            ...
            if self.timing == 'lockstep':
                ...
                if not got_response and step > 10:
                    ...
                    _total_timeouts += 1
                    logger.warning("Lockstep timeout at step %d (t=%.3f) [%d/%d]",
                                   step, t, _consecutive_timeouts, _MAX_CONSECUTIVE_TIMEOUTS)
                    print(f"[BRIDGE] \u26a0 TIMEOUT step {step} t={t:.3f}s "
                          f"[{_consecutive_timeouts}/{_MAX_CONSECUTIVE_TIMEOUTS}] "
                          f"— using stale controls")
                    if _consecutive_timeouts >= _MAX_CONSECUTIVE_TIMEOUTS:
                        logger.error("ABORT: %d consecutive lockstep timeouts — PX4 assumed dead",
                                     _MAX_CONSECUTIVE_TIMEOUTS)
                        print(f"[BRIDGE] \u274c ABORT: {_MAX_CONSECUTIVE_TIMEOUTS} consecutive "
                              f"lockstep timeouts — PX4 not responding")
                        self._running = False
                        break
                elif got_response:
                    _consecutive_timeouts = 0
        ...
        if interrupted:
            print(f"\n[BRIDGE] Interrupted at step {step}, t={t:.3f}s")
        else:
        ...
        # Export CSV explicitly (marks _csv_output=None so cleanup won't redo)
        if csv_output and len(self.history.get('time', [])) > 0:
            self._export_csv(csv_output)
            self._csv_output = None          # prevent double-export in cleanup
            print(f"[BRIDGE] Results saved to {csv_output}")

        # Cleanup socket & temp file
        self.cleanup()
    ...
    def _log_step(self, snapshot: dict, state: np.ndarray,
        ...
        for consistency with analysis labels ("X North", "Y East"). In
        long_range_mode, state[0:3] / state[3:6] are ECEF, so we use
        snapshot['pos_ned_launch'] / snapshot['vel_ned_launch'] which are
        already in the launch-fixed NED frame. Raw ECEF is recoverable
        from the lat/lon/alt_msl columns when needed.
        """
        if self.sim.long_range_mode:
            pos = np.asarray(snapshot.get('pos_ned_launch', state[0:3]))
            vel = np.asarray(snapshot.get('vel_ned_launch', state[3:6]))
        else:
            pos = state[0:3]
            vel = state[3:6]
        quat = state[6:10]
        ...
        self.history['ground_range'].append(
            snapshot.get('ground_range_km', 0.0) * 1000.0
            if self.sim.long_range_mode
            else np.sqrt(pos[0]**2 + pos[1]**2))
