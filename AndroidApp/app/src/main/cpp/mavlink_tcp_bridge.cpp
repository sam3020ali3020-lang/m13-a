...
// Watchdog heartbeat: bridge_loop updates this on every accept/poll cycle
// (at least once per second while healthy).  The /watchdog test uses this
// to distinguish a wedged-but-not-crashed TCP thread.
static std::atomic<uint64_t> s_bridge_alive_us{0};

uint64_t mavlink_tcp_bridge_alive_us() {
    return s_bridge_alive_us.load(std::memory_order_relaxed);
}
...
static void bridge_loop(int tcp_port, int udp_port) {
    ...
    while (s_bridge_running) {
        // Heartbeat: stamp every accept-loop iteration so the watchdog
        // can tell the bridge thread is alive even when no QGC is
        // connected.
        s_bridge_alive_us.store(hrt_absolute_time(), std::memory_order_relaxed);
        ...
        // Disable Nagle's algorithm — MAVLink emits small (~30-280 byte)
        // high-frequency frames; with Nagle, QGC commands and bursts of
        // telemetry get coalesced and delayed by up to 200 ms while waiting
        // on the next ACK.  Localhost has zero bandwidth concern, so the
        // extra packets cost nothing and we get sub-millisecond latency.
        int nodelay = 1;
        ...
        while (s_bridge_running) {
            // Heartbeat inside connected loop too (we don't return to the
            // outer loop while a QGC session is open).
            s_bridge_alive_us.store(hrt_absolute_time(), std::memory_order_relaxed);
...
void mavlink_tcp_bridge_start(int tcp_port, int udp_port) {
    ...
    // Note: do NOT detach.  Closing s_tcp_server_fd from another thread while
    // bridge_loop is mid accept()/poll() is racy: Linux may reuse the fd
    // number for an unrelated open before bridge_loop notices.  Keep the
    // thread joinable so stop() can wait for it to close its own fd cleanly.
}

void mavlink_tcp_bridge_stop() {
    s_bridge_running = false;
    if (s_bridge_thread.joinable()) {
        // bridge_loop checks s_bridge_running every 1 s (poll timeout) and
        // exits at the next check, closing s_tcp_server_fd inside its own
        // thread context.  Worst-case wait here is ~1 s.
        s_bridge_thread.join();
    }
    // Reset heartbeat so the watchdog observes "dead" immediately rather
    // than holding the last alive stamp until its stale threshold elapses.
    s_bridge_alive_us.store(0, std::memory_order_relaxed);
}
